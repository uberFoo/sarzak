...
impl FloatLiteral {
    ...
    pub fn new(x_value: f64, store: &mut LuDogPlMutexStore) -> Arc<Mutex<FloatLiteral>> {
        let id = Uuid::new_v4();
        let new = Arc::new(Mutex::new(FloatLiteral { id, x_value }));
    ...
    pub fn r22_literal<'a>(&'a self, store: &'a LuDogPlMutexStore) -> Vec<Arc<Mutex<Literal>>> {
        vec![store
            .iter_literal()
            .find(|literal| {
                if let LiteralEnum::FloatLiteral(id) = literal.lock().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl XPlugin {
    ...
    pub fn new(name: String, x_path: String, store: &mut LuDogPlMutexStore) -> Arc<Mutex<XPlugin>> {
        let id = Uuid::new_v4();
        let new = Arc::new(Mutex::new(XPlugin { id, name, x_path }));
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlMutexStore) -> Vec<Arc<Mutex<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::XPlugin(id) = value_type.lock().subtype {
                    id == self.id
                } else {
                    false
                }

...
use crate::v2::lu_dog_pl_mutex::types::binary::BinaryEnum;
use crate::v2::lu_dog_pl_mutex::types::or::OR;
...
impl BooleanOperator {
    ...
    pub fn new_and(bogus: bool, store: &mut LuDogPlMutexStore) -> Arc<Mutex<BooleanOperator>> {
        let id = Uuid::new_v4();
        let new = Arc::new(Mutex::new(BooleanOperator {
            bogus: bogus,
    ...
    /// Inter a new BooleanOperator in the store, and return it's `id`.
    pub fn new_or(bogus: bool, store: &mut LuDogPlMutexStore) -> Arc<Mutex<BooleanOperator>> {
        let id = Uuid::new_v4();
        let new = Arc::new(Mutex::new(BooleanOperator {
            bogus: bogus,
    ...
    pub fn r48_binary<'a>(&'a self, store: &'a LuDogPlMutexStore) -> Vec<Arc<Mutex<Binary>>> {
        vec![store
            .iter_binary()
            .find(|binary| {
                if let BinaryEnum::BooleanOperator(id) = binary.lock().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl Unary {
    ...
    pub fn new_negation(bogus: bool, store: &mut LuDogVanillaStore) -> Unary {
        let id = Uuid::new_v4();
        let new = Unary {
            bogus: bogus,
    ...
    /// Inter a new Unary in the store, and return it's `id`.
    pub fn new_not(bogus: bool, store: &mut LuDogVanillaStore) -> Unary {
        let id = Uuid::new_v4();
        let new = Unary {
            bogus: bogus,
    ...
    pub fn r47_operator<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Operator> {
        vec![store
            .iter_operator()
            .find(|operator| {
                if let OperatorEnum::Unary(id) = operator.subtype {
                    id == self.id
                } else {
                    false
                }

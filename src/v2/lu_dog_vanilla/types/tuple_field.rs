...
impl TupleField {
    ...
    pub fn new(xyzzy: Uuid, ty: &ValueType, store: &mut LuDogVanillaStore) -> TupleField {
        let id = Uuid::new_v4();
        let new = TupleField {
            id,
            xyzzy,
    ...
    pub fn r86_value_type<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&ValueType> {
    ...
    pub fn r85_enum_field<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&EnumField> {
        vec![store
            .iter_enum_field()
            .find(|enum_field| {
                if let EnumFieldEnum::TupleField(id) = enum_field.subtype {
                    id == self.id
                } else {
                    false
                }

...
use crate::v2::lu_dog_vanilla::types::statement::StatementEnum;
...
impl ExpressionStatement {
    ...
    pub fn new(expression: &Expression, store: &mut LuDogVanillaStore) -> ExpressionStatement {
        let id = Uuid::new_v4();
    ...
    pub fn r31_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
    ...
    pub fn r16_statement<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Statement> {
        vec![store
            .iter_statement()
            .find(|statement| {
                if let StatementEnum::ExpressionStatement(id) = statement.subtype {
                    id == self.id
                } else {
                    false
                }

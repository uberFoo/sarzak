...
impl LocalVariable {
    ...
    pub fn new(bug: Uuid, store: &mut LuDogVanillaStore) -> LocalVariable {
        let id = Uuid::new_v4();
        let new = LocalVariable { bug, id };
    ...
    pub fn r21_let_statement<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&LetStatement> {
        vec![store
            .iter_let_statement()
    ...
    pub fn r12_variable<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Variable> {
        vec![store
            .iter_variable()
            .find(|variable| {
                if let VariableEnum::LocalVariable(id) = variable.subtype {
                    id == self.id
                } else {
                    false
                }

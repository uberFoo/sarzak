...
pub fn new_z_object_store(z_object_store: &ZObjectStore, store: &mut LuDogVanillaStore) -> Self {
...
store.iter_woog_option()
.filter(|woog_option| {
woog_option.ty == self.id()
})
.collect()
}
// {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
...
store.iter_reference()
.filter(|reference| {
reference.ty == self.id()
})
.collect()
}
// {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
...
store.iter_span()
.filter_map(|span| if span.ty == Some(self.id()) { Some(span) } else { None })
.collect()
}
// {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
...
store.iter_type_cast()
.filter(|type_cast| {
type_cast.ty == self.id()
})
.collect()
}
// {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}

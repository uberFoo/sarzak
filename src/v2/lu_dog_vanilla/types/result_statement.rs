...
impl ResultStatement {
    ...
    pub fn new(expression: &Expression, store: &mut LuDogVanillaStore) -> ResultStatement {
        let id = Uuid::new_v4();
    ...
    pub fn r41_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
    ...
    pub fn r16_statement<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Statement> {
        vec![store
            .iter_statement()
            .find(|statement| {
                if let StatementEnum::ResultStatement(id) = statement.subtype {
                    id == self.id
                } else {
                    false
                }

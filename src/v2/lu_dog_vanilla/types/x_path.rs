...
use crate::v2::lu_dog_vanilla::types::path_element::PathElement;
...
impl XPath {
    ...
    pub fn new(unique: Uuid, first: Option<&PathElement>, store: &mut LuDogVanillaStore) -> XPath {
        let id = Uuid::new_v4();
        let new = XPath {
            id,
            unique,
            first: first.as_ref().map(|path_element| path_element.id),
        };
    ...
    pub fn r97_path_element<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&PathElement> {
        match self.first {
    ...
    pub fn r90_path_element<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&PathElement> {
        store
            .iter_path_element()
            .filter(|path_element| path_element.x_path == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r96_struct_expression<'a>(
        ...
        store
            ...
            .filter(|struct_expression| struct_expression.x_path == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r15_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::XPath(id) = expression.subtype {
                    id == self.id
                } else {
                    false
                }

...
impl AWait {
    ...
    pub fn new(x_future: &Expression, store: &mut LuDogVanillaStore) -> AWait {
        let id = Uuid::new_v4();
        let new = AWait {
            id,
            x_future: x_future.id,
        };
    ...
    pub fn r98_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
    ...
    pub fn r15_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::AWait(id) = expression.subtype {
                    id == self.id
                } else {
                    false
                }

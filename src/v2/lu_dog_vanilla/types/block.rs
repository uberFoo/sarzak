...
impl Block {
    ...
    pub fn new(
        ...
        let new = Block {
            ...
            bug,
            id,
            parent: parent.as_ref().map(|block| block.id),
            statement: statement.as_ref().map(|statement| statement.id),
        };
    ...
    pub fn r93_block<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Block> {
        match self.parent {
    ...
    pub fn r93c_block<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Block> {
        let block = store
            .iter_block()
            .find(|block| block.parent == Some(self.id));
        match block {
    ...
    pub fn r80_body<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Body> {
        vec![store
            .iter_body()
            .find(|body| {
                if let BodyEnum::Block(id) = body.subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r15_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::Block(id) = expression.subtype {
                    id == self.id
                } else {
                    false
                }

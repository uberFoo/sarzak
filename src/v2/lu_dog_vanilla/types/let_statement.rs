...
impl LetStatement {
    ...
    pub fn r20_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
    ...
    pub fn r21_local_variable<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&LocalVariable> {
    ...
    pub fn r16_statement<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Statement> {
        vec![store
            .iter_statement()
            .find(|statement| {
                if let StatementEnum::LetStatement(id) = statement.subtype {
                    id == self.id
                } else {
                    false
                }

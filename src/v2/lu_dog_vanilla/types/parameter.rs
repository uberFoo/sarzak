...
impl Parameter {
    ...
    pub fn r79_value_type<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&ValueType> {
    ...
    pub fn r82c_function<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Function> {
        let function = store
            .iter_function()
            .find(|function| function.first_param == Some(self.id));
        match function {

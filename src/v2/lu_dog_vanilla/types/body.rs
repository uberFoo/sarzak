...
use crate::v2::lu_dog_vanilla::types::external_implementation::ExternalImplementation;
...
impl Body {
    ...
    pub fn new_block(a_sink: bool, subtype: &Block, store: &mut LuDogVanillaStore) -> Body {
        let id = Uuid::new_v4();
        let new = Body {
            a_sink: a_sink,
            subtype: BodyEnum::Block(subtype.id),
            id,
        };
    ...
    pub fn new_external_implementation(
        a_sink: bool,
        subtype: &ExternalImplementation,
        store: &mut LuDogVanillaStore,
    ) -> Body {
        let id = Uuid::new_v4();
        let new = Body {
            a_sink: a_sink,
            subtype: BodyEnum::ExternalImplementation(subtype.id),
            id,
        };
        store.inter_body(new.clone());
        new
    ...
    pub fn r19c_function<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Function> {
        let function = store
            .iter_function()
            .find(|function| function.body == self.id);
        match function {
    ...
    pub fn r73c_lambda<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Lambda> {
        let lambda = store
            .iter_lambda()
            .find(|lambda| lambda.body == Some(self.id));
        match lambda {
            Some(ref lambda) => vec![lambda],
            None => Vec::new(),
        }
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}

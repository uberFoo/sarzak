...
impl Index {
    ...
    pub fn new(index: &Expression, target: &Expression, store: &mut LuDogVanillaStore) -> Index {
        let id = Uuid::new_v4();
        let new = Index {
            id,
            index: index.id(),
    ...
    pub fn r56_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {
    ...
    pub fn r57_expression<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Expression> {

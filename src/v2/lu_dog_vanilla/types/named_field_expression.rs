...
impl NamedFieldExpression {
    ...
    pub fn new(name: String, store: &mut LuDogVanillaStore) -> NamedFieldExpression {
        let id = Uuid::new_v4();
        let new = NamedFieldExpression { id, name };
    ...
    pub fn r94_field_expression<'a>(
        ...
        vec![store
            ...
            .find(|field_expression| {
                if let FieldExpressionEnum::NamedFieldExpression(id) = field_expression.subtype {
                    id == self.id
                } else {
                    false
                }

...
impl LambdaParameter {
    ...
    pub fn new(
        ...
        let id = Uuid::new_v4();
        let new = LambdaParameter {
            id,
            position,
            lambda: lambda.id,
            next: next.as_ref().map(|lambda_parameter| lambda_parameter.id),
    ...
    pub fn r76_lambda<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Lambda> {
    ...
    pub fn r77_value_type<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&ValueType> {
        match self.ty {
    ...
    pub fn r103_lambda<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Lambda> {
        vec![store
            .iter_lambda()
            .find(|lambda| lambda.first_param == Some(self.id))
            .unwrap()]
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r75c_lambda_parameter<'a>(
        ...
        let lambda_parameter = store
            ...
            .find(|lambda_parameter| lambda_parameter.next == Some(self.id));
        match lambda_parameter {
            Some(ref lambda_parameter) => vec![lambda_parameter],
            None => Vec::new(),
        }
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r12_variable<'a>(&'a self, store: &'a LuDogVanillaStore) -> Vec<&Variable> {
        vec![store
            .iter_variable()
            .find(|variable| {
                if let VariableEnum::LambdaParameter(id) = variable.subtype {
                    id == self.id
                } else {
                    false
                }

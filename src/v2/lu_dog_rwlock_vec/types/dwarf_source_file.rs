...
impl DwarfSourceFile {
    ...
    pub fn new(source: String, store: &mut LuDogRwlockVecStore) -> Arc<RwLock<DwarfSourceFile>> {
    ...
    pub fn r25_item<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<Item>>> {
        span!("r25_item");
        store
            .iter_item()
    ...
    pub fn r64_span<'a>(&'a self, store: &'a LuDogRwlockVecStore) -> Vec<Arc<RwLock<Span>>> {
        span!("r64_span");
        store
            .iter_span()

...
impl XIf {
    ...
    pub fn new(
        ...
        store.inter_x_if(|id| {
            Arc::new(RwLock::new(XIf {
                id,
                false_block: false_block.map(|expression| expression.read().id),
                test: test.read().id,
    ...
    pub fn r46_block<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Block>>> {
    ...
    pub fn r15_expression<'a>(
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::XIf(id) = expression.read().subtype {
                    id == self.id
                } else {
                    false
                }

...
use crate::v2::lu_dog_pl_vec::types::statement::StatementEnum;
...
impl ResultStatement {
    ...
    pub fn r16_statement<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Statement>>> {
        vec![store
            .iter_statement()
            .find(|statement| {
                if let StatementEnum::ResultStatement(id) = statement.read().subtype {
                    id == self.id
                } else {
                    false
                }

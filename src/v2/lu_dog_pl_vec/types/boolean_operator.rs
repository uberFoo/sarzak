...
use crate::v2::lu_dog_pl_vec::types::and::AND;
use crate::v2::lu_dog_pl_vec::types::binary::Binary;
use crate::v2::lu_dog_pl_vec::types::binary::BinaryEnum;
use crate::v2::lu_dog_pl_vec::types::or::OR;
...
impl BooleanOperator {
    ...
    pub fn new_and(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<BooleanOperator>> {
    ...
    /// Inter a new BooleanOperator in the store, and return it's `id`.
    pub fn new_or(bogus: bool, store: &mut LuDogPlVecStore) -> Arc<RwLock<BooleanOperator>> {
        store.inter_boolean_operator(|id| {
            Arc::new(RwLock::new(BooleanOperator {
                bogus: bogus,
    ...
    pub fn r48_binary<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Binary>>> {
        vec![store
            .iter_binary()
            .find(|binary| {
                if let BinaryEnum::BooleanOperator(id) = binary.read().subtype {
                    id == self.id
                } else {
                    false
                }

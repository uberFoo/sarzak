...
impl List {
    ...
    pub fn new(ty: &Arc<RwLock<ValueType>>, store: &mut LuDogPlVecStore) -> Arc<RwLock<List>> {
        store.inter_list(|id| {
    ...
    pub fn r36_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::List(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }

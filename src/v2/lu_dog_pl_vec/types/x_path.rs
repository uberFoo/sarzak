...
use crate::v2::lu_dog_pl_vec::types::path_element::PathElement;
...
impl XPath {
    ...
    pub fn new(
        ...
        store.inter_x_path(|id| {
            Arc::new(RwLock::new(XPath {
                id,
                unique,
                first: first.map(|path_element| path_element.read().id),
            }))
        })
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r90_path_element<'a>(
        ...
        store
            .iter_path_element()
            .filter(|path_element| path_element.read().x_path == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r96_struct_expression<'a>(
        ...
        store
            ...
            .filter(|struct_expression| struct_expression.read().x_path == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r15_expression<'a>(
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::XPath(id) = expression.read().subtype {
                    id == self.id
                } else {
                    false
                }

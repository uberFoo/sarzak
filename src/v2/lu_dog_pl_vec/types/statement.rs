...
use crate::v2::lu_dog_pl_vec::types::item_statement::ITEM_STATEMENT;
...
impl Statement {
    ...
    pub fn new_expression_statement(
        ...
        store.inter_statement(|id| {
            Arc::new(RwLock::new(Statement {
                ...
                subtype: StatementEnum::ExpressionStatement(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_let_statement(
        ...
        store.inter_statement(|id| {
            Arc::new(RwLock::new(Statement {
                ...
                block: block.read().id,
                next: next.map(|statement| statement.read().id),
                subtype: StatementEnum::LetStatement(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_result_statement(
        ...
        store.inter_statement(|id| {
            Arc::new(RwLock::new(Statement {
                ...
                block: block.read().id,
                next: next.map(|statement| statement.read().id),
                subtype: StatementEnum::ResultStatement(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn r18_block<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Block>>> {
    ...
    pub fn r17_statement<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Statement>>> {
        match self.next {
    ...
    pub fn r71c_block<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Block>>> {
        let block = store
            .iter_block()
            .find(|block| block.read().statement == Some(self.id));
        match block {
    ...
    pub fn r17c_statement<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Statement>>> {
        let statement = store
            .iter_statement()

...
use crate::v2::lu_dog_pl_vec::types::object_wrapper::ObjectWrapper;
...
impl ZObjectStore {
    ...
    pub fn r83c_implementation_block<'a>(
        ...
        let implementation_block = store
            ...
            .find(|implementation_block| implementation_block.read().object_store == Some(self.id));
    ...
    pub fn r78_object_wrapper<'a>(
        ...
        store
            .iter_object_wrapper()
            .filter(|object_wrapper| object_wrapper.read().z_store == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::ZObjectStore(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl Argument {
    ...
    pub fn r28_call<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Call>>> {
    ...
    pub fn r27_argument<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Argument>>> {
        match self.next {
    ...
    pub fn r27c_argument<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Argument>>> {
        let argument = store
            .iter_argument()
    ...
    pub fn r81c_call<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Call>>> {
        let call = store
            .iter_call()
            .find(|call| call.read().argument == Some(self.id));
        match call {

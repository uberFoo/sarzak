...
impl Span {
    ...
    pub fn r62_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        match self.ty {
    ...
    pub fn r63_x_value<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<XValue>>> {
        match self.x_value {

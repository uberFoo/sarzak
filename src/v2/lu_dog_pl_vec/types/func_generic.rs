...
impl FuncGeneric {
    ...
    pub fn new(
        ...
        store.inter_func_generic(|id| {
            Arc::new(RwLock::new(FuncGeneric {
                id,
                name: name.to_owned(),
                func: func.map(|function| function.read().id),
                next: next.map(|func_generic| func_generic.read().id),
            }))
        })
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r107_function<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Function>>> {
        match self.func {
    ...
    pub fn r3c_func_generic<'a>(
        ...
        let func_generic = store
            .iter_func_generic()
            .find(|func_generic| func_generic.read().next == Some(self.id));
        match func_generic {
    ...
    pub fn r99_function<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Function>>> {
        vec![store
            .iter_function()
            .find(|function| function.read().first_generic == Some(self.id))
            .unwrap()]
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::FuncGeneric(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }

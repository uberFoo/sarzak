...
impl Lambda {
    ...
    pub fn new(
        ...
        store.inter_lambda(|id| {
            Arc::new(RwLock::new(Lambda {
                id,
                body: body.map(|body| body.read().id),
                first_param: first_param.map(|lambda_parameter| lambda_parameter.read().id),
    ...
    pub fn r73_body<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Body>>> {
        match self.body {
    ...
    pub fn r74_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
    ...
    pub fn r76_lambda_parameter<'a>(
        ...
        store
            ...
            .filter(|lambda_parameter| lambda_parameter.read().lambda == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r15_expression<'a>(
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::Lambda(id) = expression.read().subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::Lambda(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }

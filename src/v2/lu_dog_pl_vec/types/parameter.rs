...
impl Parameter {
    ...
    pub fn r13_function<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Function>>> {
    ...
    pub fn r14_parameter<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Parameter>>> {
        match self.next {
    ...
    pub fn r79_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
    ...
    pub fn r82c_function<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Function>>> {
        let function = store
            .iter_function()
            .find(|function| function.read().first_param == Some(self.id));
        match function {
    ...
    pub fn r14c_parameter<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Parameter>>> {
        let parameter = store
            .iter_parameter()

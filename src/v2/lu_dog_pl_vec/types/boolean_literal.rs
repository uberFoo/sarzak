...
use crate::v2::lu_dog_pl_vec::types::false_literal::FALSE_LITERAL;
use crate::v2::lu_dog_pl_vec::types::literal::Literal;
use crate::v2::lu_dog_pl_vec::types::literal::LiteralEnum;
use crate::v2::lu_dog_pl_vec::types::true_literal::TRUE_LITERAL;
...
impl BooleanLiteral {
    ...
    pub fn r22_literal<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Literal>>> {
        vec![store
            .iter_literal()
            .find(|literal| {
                if let LiteralEnum::BooleanLiteral(id) = literal.read().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl StringLiteral {
    ...
    pub fn new(x_value: String, store: &mut LuDogPlVecStore) -> Arc<RwLock<StringLiteral>> {
    ...
    pub fn r108_string_bit<'a>(
        ...
        store
            .iter_string_bit()
            .filter(|string_bit| string_bit.read().z_string == self.id)
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r22_literal<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Literal>>> {
        vec![store
            .iter_literal()
            .find(|literal| {
                if let LiteralEnum::StringLiteral(id) = literal.read().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl DwarfSourceFile {
    ...
    pub fn new(source: String, store: &mut LuDogPlVecStore) -> Arc<RwLock<DwarfSourceFile>> {
    ...
    pub fn r25_item<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Item>>> {
        store
            .iter_item()
    ...
    pub fn r64_span<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Span>>> {
        store
            .iter_span()

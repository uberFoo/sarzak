...
impl Function {
    ...
    pub fn new(
        ...
        store.inter_function(|id| {
            Arc::new(RwLock::new(Function {
                ...
                name: name.to_owned(),
                body: body.read().id,
                first_generic: first_generic.map(|func_generic| func_generic.read().id),
                first_param: first_param.map(|parameter| parameter.read().id),
                impl_block: impl_block.map(|implementation_block| implementation_block.read().id),
    ...
    pub fn r19_body<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Body>>> {
    ...
    pub fn r82_parameter<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Parameter>>> {
    ...
    pub fn r10_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
    ...
    pub fn r107_func_generic<'a>(
        ...
        store
            .iter_func_generic()
            .filter(|func_generic| func_generic.read().func == Some(self.id))
            .collect()
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r13_parameter<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Parameter>>> {
        store
            .iter_parameter()
    ...
    pub fn r67_field_access_target<'a>(
        ...
        vec![store
            ...
            .find(|field_access_target| {
                if let FieldAccessTargetEnum::Function(id) = field_access_target.read().subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r1_value_type<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<ValueType>>> {
        vec![store
            .iter_value_type()
            .find(|value_type| {
                if let ValueTypeEnum::Function(id) = value_type.read().subtype {
                    id == self.id
                } else {
                    false
                }

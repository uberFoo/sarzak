...
use crate::v2::lu_dog_pl_vec::types::body::Body;
use crate::v2::lu_dog_pl_vec::types::body::BodyEnum;
use crate::v2::lu_dog_pl_vec::types::expression::Expression;
use crate::v2::lu_dog_pl_vec::types::expression::ExpressionEnum;
use crate::v2::lu_dog_pl_vec::types::statement::Statement;
...
impl Block {
    ...
    pub fn new(
        ...
        store.inter_block(|id| {
            Arc::new(RwLock::new(Block {
                ...
                bug,
                id,
                parent: parent.map(|block| block.read().id),
                statement: statement.map(|statement| statement.read().id),
            }))
        })
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r93_block<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Block>>> {
        match self.parent {
    ...
    pub fn r71_statement<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Statement>>> {
    ...
    pub fn r93c_block<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Block>>> {
        let block = store
            .iter_block()
            .find(|block| block.read().parent == Some(self.id));
        match block {
    ...
    pub fn r46_x_if<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<XIf>>> {
        store
            .iter_x_if()
    ...
    pub fn r18_statement<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Statement>>> {
        store
            .iter_statement()
    ...
    pub fn r33_x_value<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<XValue>>> {
        store
            .iter_x_value()
    ...
    pub fn r80_body<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Body>>> {
        vec![store
            .iter_body()
            .find(|body| {
                if let BodyEnum::Block(id) = body.read().subtype {
                    id == self.id
                } else {
                    false
                }
    ...
    pub fn r15_expression<'a>(
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::Block(id) = expression.read().subtype {
                    id == self.id
                } else {
                    false
                }

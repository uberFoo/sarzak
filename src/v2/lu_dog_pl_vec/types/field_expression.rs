...
use crate::v2::lu_dog_pl_vec::types::named_field_expression::NamedFieldExpression;
use crate::v2::lu_dog_pl_vec::types::struct_expression::StructExpression;
use crate::v2::lu_dog_pl_vec::types::unnamed_field_expression::UnnamedFieldExpression;
...
impl FieldExpression {
    ...
    pub fn new_named_field_expression(
        ...
        store.inter_field_expression(|id| {
            Arc::new(RwLock::new(FieldExpression {
                ...
                subtype: FieldExpressionEnum::NamedFieldExpression(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn new_unnamed_field_expression(
        ...
        store.inter_field_expression(|id| {
            Arc::new(RwLock::new(FieldExpression {
                expression: expression.read().id,
                woog_struct: woog_struct.read().id,
                subtype: FieldExpressionEnum::UnnamedFieldExpression(subtype.read().id), // b
                id,
            }))
        })
    }
    ...
    pub fn r15_expression<'a>(
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::FieldExpression(id) = expression.read().subtype {
                    id == self.id
                } else {
                    false
                }

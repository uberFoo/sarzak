...
use crate::v2::lu_dog_pl_vec::types::call::CallEnum;
...
impl FunctionCall {
    ...
    pub fn new(name: String, store: &mut LuDogPlVecStore) -> Arc<RwLock<FunctionCall>> {
    ...
    pub fn r30_call<'a>(&'a self, store: &'a LuDogPlVecStore) -> Vec<Arc<RwLock<Call>>> {
        vec![store
            .iter_call()
            .find(|call| {
                if let CallEnum::FunctionCall(id) = call.read().subtype {
                    id == self.id
                } else {
                    false
                }

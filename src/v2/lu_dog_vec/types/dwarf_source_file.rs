...
impl DwarfSourceFile {
    ...
    pub fn new(source: String, store: &mut LuDogVecStore) -> Rc<RefCell<DwarfSourceFile>> {
    ...
    pub fn r25_item<'a>(&'a self, store: &'a LuDogVecStore) -> Vec<Rc<RefCell<Item>>> {
        span!("r25_item");
        store
            .iter_item()
    ...
    pub fn r64_span<'a>(&'a self, store: &'a LuDogVecStore) -> Vec<Rc<RefCell<Span>>> {
        span!("r64_span");
        store
            .iter_span()

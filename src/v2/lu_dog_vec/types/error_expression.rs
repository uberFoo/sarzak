...
impl ErrorExpression {
    ...
    pub fn new(span: String, store: &mut LuDogVecStore) -> Rc<RefCell<ErrorExpression>> {
        store.inter_error_expression(|id| {
            Rc::new(RefCell::new(ErrorExpression {
                id,
                span: span.to_owned(),
            }))
        })
    ...
    pub fn r15_expression<'a>(&'a self, store: &'a LuDogVecStore) -> Vec<Rc<RefCell<Expression>>> {
        ...
        vec![store
            .iter_expression()
            .find(|expression| {
                if let ExpressionEnum::ErrorExpression(id) = expression.borrow().subtype {
                    id == self.id
                } else {
                    false
                }

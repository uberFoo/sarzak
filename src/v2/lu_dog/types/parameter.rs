...
impl Parameter {
    ...
    pub fn new(function: &Function, next: Option<&Parameter>, store: &mut LuDogStore) -> Parameter {
        let id = Uuid::new_v4();
        let new = Parameter {
            id: id,
    ...
    /// Inter a new 'Parameter' in the store, and return it's `id`.
    pub fn new_(function: &Function, next: Option<&Parameter>) -> Parameter {
        let id = Uuid::new_v4();
        let new = Parameter {
            id: id,
            function: function.id,
    ...
    pub fn r13_function<'a>(&'a self, store: &'a LuDogStore) -> Vec<&Function> {
    ...
    pub fn r14_parameter<'a>(&'a self, store: &'a LuDogStore) -> Vec<&Parameter> {
        match self.next {
    ...
    pub fn r14c_parameter<'a>(&'a self, store: &'a LuDogStore) -> Vec<&Parameter> {
        let parameter = store
            .iter_parameter()

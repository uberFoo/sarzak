...
impl Statement {
    ...
    pub fn new_result_statement(
        ...
        // 🚧 I'm not using id below with subtype because that's rendered where it doesn't know
        // about this local. This should be fixed in the near future.
        let id = RESULT_STATEMENT;
        let new = Statement {
            block: block.id,
            next: next.map(|statement| statement.id),
            subtype: StatementEnum::ResultStatement(RESULT_STATEMENT),
            id,
        };
        store.inter_statement(new.clone());
        new
    ...
    /// Inter a new Statement in the store, and return it's `id`.
    pub fn new_result_statement_(block: &Block, next: Option<&Statement>) -> Statement {
        // 🚧 I'm not using id below with subtype because that's rendered where it doesn't know
        // about this local. This should be fixed in the near future.
        let id = RESULT_STATEMENT;
        let new = Statement {
    ...
    pub fn r17_statement<'a>(&'a self, store: &'a LuDogStore) -> Vec<&Statement> {
        match self.next {
    ...
    pub fn r17c_statement<'a>(&'a self, store: &'a LuDogStore) -> Vec<&Statement> {
        let statement = store
            .iter_statement()

...
impl VariableExpression {
    ...
    pub async fn new(name: String, store: &mut LuDogAsyncStore) -> Arc<RwLock<VariableExpression>> {
        store
            .inter_variable_expression(|id| {
    ...
    pub async fn r15_expression<'a>(
        ...
        store
            ...
            .filter_map(|expression| async move {
                if let ExpressionEnum::VariableExpression(id) = expression.read().await.subtype {

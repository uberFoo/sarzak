...
impl PathElement {
    ...
    pub fn r90_x_path<'a>(&'a self, store: &'a LuDogNdrwlockVecStore) -> Vec<Arc<RwLock<XPath>>> {
        span!("r90_x_path");
    ...
    pub fn r97_x_path<'a>(&'a self, store: &'a LuDogNdrwlockVecStore) -> Vec<Arc<RwLock<XPath>>> {
        span!("r97_x_path");
        vec![store
            .iter_x_path()

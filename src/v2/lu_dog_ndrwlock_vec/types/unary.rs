...
impl Unary {
    ...
    pub fn new_negation(bogus: bool, store: &mut LuDogNdrwlockVecStore) -> Arc<RwLock<Unary>> {
    ...
    /// Inter a new Unary in the store, and return it's `id`.
    pub fn new_not(bogus: bool, store: &mut LuDogNdrwlockVecStore) -> Arc<RwLock<Unary>> {
        store.inter_unary(|id| {
            Arc::new(RwLock::new(Unary {
                bogus: bogus,

...
impl IsaUi {
    ...
    pub fn new(
        ...
        let id = Uuid::new_v4();
        let new = Rc::new(RefCell::new(IsaUi {
            id,
            from: from.borrow().id,
            isa: isa.id,
        }));
    ...
    pub fn r9_anchor<'a>(&'a self, store: &'a DrawingStore) -> Vec<Rc<RefCell<Anchor>>> {
        span!("r9_anchor");
    ...
    pub fn r11_isa<'a>(&'a self, store: &'a SarzakStore) -> Vec<Rc<RefCell<Isa>>> {
    ...
    pub fn r10_subtype_anchors<'a>(
        ...
        let subtype_anchors = store
            ...
            .find(|subtype_anchors| subtype_anchors.borrow().isaui_id == self.id);

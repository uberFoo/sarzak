...
use crate::v2::lu_dog_vec_tracy::types::variable::VariableEnum;
...
impl LambdaParameter {
    ...
    pub fn r76_lambda<'a>(&'a self, store: &'a LuDogVecTracyStore) -> Vec<Rc<RefCell<Lambda>>> {
        span!("r76_lambda");
    ...
    pub fn r103_lambda<'a>(&'a self, store: &'a LuDogVecTracyStore) -> Vec<Rc<RefCell<Lambda>>> {
        span!("r103_lambda");
        vec![store
            .iter_lambda()
    ...
    pub fn r12_variable<'a>(&'a self, store: &'a LuDogVecTracyStore) -> Vec<Rc<RefCell<Variable>>> {
        span!("r12_variable");
        vec![store
            .iter_variable()
            .find(|variable| {

...
use crate::v2::lu_dog_vec_tracy::types::enum_field::EnumFieldEnum;
...
impl Unit {
    ...
    pub fn new(x_value: i64, store: &mut LuDogVecTracyStore) -> Rc<RefCell<Unit>> {
        store.inter_unit(|id| Rc::new(RefCell::new(Unit { id, x_value })))
    }
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    ...
    pub fn r85_enum_field<'a>(
        ...
        vec![store
            .iter_enum_field()
            .find(|enum_field| {
                if let EnumFieldEnum::Unit(id) = enum_field.borrow().subtype {
                    id == self.id
                } else {
                    false
                }

...
impl Block {
    ...
    pub fn new(seed: Uuid, store: &mut WoogSingleStore) -> Block {
        let id = Uuid::new_v4();
    ...
    pub fn r23c_object_method<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&ObjectMethod> {
    ...
    pub fn r12_statement<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&Statement> {
        store
            .iter_statement()
    ...
    pub fn r24_symbol_table<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&SymbolTable> {
        vec![store
            .iter_symbol_table()

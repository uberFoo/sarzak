...
impl Parameter {
    ...
    pub fn new(
        ...
        let new = Parameter {
            id,
            seed,
            function: function.as_ref().map(|function| function.id),
    ...
    pub fn r5_function<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&Function> {
        match self.function {
    ...
    pub fn r1_parameter<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&Parameter> {
        match self.next {
    ...
    pub fn r1c_parameter<'a>(&'a self, store: &'a WoogSingleStore) -> Vec<&Parameter> {
        let parameter = store
            .iter_parameter()

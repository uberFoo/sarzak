...
impl SubtypeAnchors {
    ...
    /// Inter a new SubtypeAnchors in the store, and return it's `id`.
    pub fn new(isaui_id: &IsaUi, anchor_id: &Anchor, store: &mut DrawingV2Store) -> SubtypeAnchors {
        let id = Uuid::new_v5(
            &UUID_NS,
    ...
    pub fn r10_isa_ui<'a>(&'a self, store: &'a DrawingV2Store) -> Vec<&IsaUi> {
    ...
    pub fn r10_anchor<'a>(&'a self, store: &'a DrawingV2Store) -> Vec<&Anchor> {

    ...
    recursive(|expr| {
        let raw_expr = recursive(|raw_expr| {
            ...
            let for_loop = just(Token::For)
                ...
                .debug("for")
                .labelled("for loop variable")
                .then_ignore(just(Token::In))
                .then(expr.clone())
                .labelled("for loop iterable")
                .then(
                    // stmt_parser()
                    expr.debug("for loop body")
                        // .map_with_span(|stmt, span| (stmt, span))
                        .repeated()
                        .delimited_by(just(Token::Punct('{')), just(Token::Punct('}')))
                        .labelled("for loop body"),
                )
                .map_with_span(|((var, iterable), body), span: Span| {
                    (Expression::For(var, Box::new(iterable), body), span)
                });

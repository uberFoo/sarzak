...
impl Binary {
    ...
    pub fn new(number: i64, to: &Referent, from: &Referrer, store: &mut SarzakV2Store) -> Binary {
        let id = Uuid::new_v5(
            &UUID_NS,
    ...
    pub fn r5_referent<'a>(&'a self, store: &'a SarzakV2Store) -> Vec<&Referent> {
    ...
    pub fn r6_referrer<'a>(&'a self, store: &'a SarzakV2Store) -> Vec<&Referrer> {

...
impl Object {
    ...
    // {"magic":"","directive":{"End":{"directive":"ignore-orig"}}}
    // {"magic":"","directive":{"Start":{"directive":"ignore-orig","tag":"object-struct-impl-nav-backward-1_M-to-object_method"}}}
    /// Navigate to [`ObjectMethod`] across R4(1-M)
    pub fn r4_object_method<'a>(&'a self, store: &'a WoogV2Store) -> Vec<&ObjectMethod> {
        store
            .iter_object_method()
            .filter_map(|object_method| {
                if object_method.1.object == self.id {

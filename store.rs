...
pub struct ObjectStore {
    ...
    r13_parameter: Arc<RwLock<HashMap<Uuid, Vec<Arc<RwLock<Parameter>>>>>>,
    r63_span: Arc<RwLock<HashMap<Uuid, Arc<RwLock<Span>>>>>,
}
...
impl ObjectStore {
    pub fn new() -> Self {
        let mut store = Self {
            ...
            r13_parameter: Arc::new(RwLock::new(HashMap::default())),
            r63_span: Arc::new(RwLock::new(HashMap::default())),
        };
    ...
    pub fn inter_r63_span(&mut self, span: Arc<RwLock<Span>>) {
        let read = span.read().unwrap();
        self.r63_span.write().unwrap().insert(read.id, span.clone());
    }

    pub fn exhume_r63_span(&self, id: &Uuid) -> Option<Arc<RwLock<Span>>> {
        self.r63_span
            .read()
            .unwrap()
            .get(id)
    ...
    pub fn inter_parameter(&mut self, parameter: Arc<RwLock<Parameter>>) {
        ...
        self.r13_parameter
            .write()
            .unwrap()
            .entry(parameter.read().unwrap().function)
            .and_modify(|vec| vec.push(parameter.clone()))
            .or_insert(vec![parameter.clone()]);
    }

    pub fn exhume_r13_parameter(&self, id: &Uuid) -> Option<Vec<Arc<RwLock<Parameter>>>> {
        self.r13_parameter
            .read()
            .unwrap()
            .get(id)
    ...
    pub fn persist<P: AsRef<Path>>(&self, path: P) -> io::Result<()> {
        ...
        // Persist Negation.
        {
            ...
            fs::create_dir_all(&path)?;
            for negation_tuple in self.negation.read().unwrap().values() {
                let path = path.join(format!("{}.json", negation_tuple.0.read().unwrap().id));
                if path.exists() {
                    let file = fs::File::open(&path)?;
                    let reader = io::BufReader::new(file);
                    let on_disk: (Arc<RwLock<Negation>>, SystemTime) =
                        serde_json::from_reader(reader)?;
                    if on_disk.0.read().unwrap().to_owned()
                        != negation_tuple.0.read().unwrap().to_owned()
                    {
                        let file = fs::File::create(path)?;
                        let mut writer = io::BufWriter::new(file);
            ...
            for file in fs::read_dir(&path)? {
                ...
                let id = file_name.split('.').next().unwrap();
                if let Ok(id) = Uuid::parse_str(id) {
                    if !self.negation.read().unwrap().contains_key(&id) {
                        fs::remove_file(path)?;
                    }
                }
            }
        ...
        // Load Negation.
        {
            ...
            for entry in entries {
                ...
                store
                    ...
                    .write()
                    .unwrap()
                    .insert(negation.0.read().unwrap().id, negation.clone());
            }
        }
